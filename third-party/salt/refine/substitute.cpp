...
// void time(ista::TimingEngine *_timing_engine, ista::Net *net, salt::Tree& saltTree) {
...
//         double unitCap = dynamic_cast<TimingIDBAdapter *>(db_adapter)
//                     ->getCapacitance(1, 1.0/_dbu, width);

//         // unitCapacitance = cap;
//         double unitRes = dynamic_cast<TimingIDBAdapter *>(db_adapter)
//                           ->getResistance(1, 1.0/_dbu, width);
...
void Refine::Substitute_delay_aware_1(Tree& tree, double eps, double rd, bool useRTree) {
    bgi::rtree<RNode, bgi::rstar<8>, bgi::indexable<RNode>, RNodeComp> rtree;
    if (useRTree) {
        tree.postOrder([&](const shared_ptr<TreeNode>& n) {
            if (n->parent) {
...
void Refine::Substitute_delay_aware(Tree& tree, double eps, double rd, bool useRTree) {
    bgi::rtree<RNode, bgi::rstar<8>, bgi::indexable<RNode>, RNodeComp> rtree;
    if (useRTree) {
        tree.postOrder([&](const shared_ptr<TreeNode>& n) {
            if (n->parent) {
...
shared_ptr<TreeNode> findLowestCommonAncestor(shared_ptr<TreeNode> p, shared_ptr<TreeNode> q) {
...
// p=1, q=3
double calc_delay_delta(shared_ptr<TreeNode> p,
    ...
    switch (which_case) {
        ...
        case Which_Case::c: {
            auto ancestor = findLowestCommonAncestor(critical_tmp, p_q_ancestor);
...
void Refine::Substitute_critical_delay_aware(Tree& tree, double eps, double rd, bool useRTree) {
    bgi::rtree<RNode, bgi::rstar<8>, bgi::indexable<RNode>, RNodeComp> rtree;
    if (useRTree) {
        tree.postOrder([&](const shared_ptr<TreeNode>& n) {
            if (n->parent) {
    ...
    if (firstDigitInt > 5) firstDigitInt = 1;
        ...
        for (auto node : nodes) {
            ...
            for (int neighIdx : nearestNeighbors[node->id]) {
                ...
                auto delay_delta = calc_delay_delta(node->parent,
                                                    neighParent,
                                                    critical_node,
                                                    node,
                                                    neigh,
...
void Refine::Substitute_critical_delay_aware_1(Tree& tree, double eps, double rd, bool useRTree) {
    bgi::rtree<RNode, bgi::rstar<8>, bgi::indexable<RNode>, RNodeComp> rtree;
    if (useRTree) {
        tree.postOrder([&](const shared_ptr<TreeNode>& n) {
            if (n->parent) {
